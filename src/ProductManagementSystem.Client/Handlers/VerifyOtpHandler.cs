...
public class VerifyOtpHandler
{
    ...
    public VerifyOtpHandler(IHttpClientFactory httpClientFactory, IConfiguration configuration)
    {
    ...
    public async Task<(bool isSuccessful, string responseMessage)> Handle(ValidateOtpRequestDto validateOtpRequest)
    {
        try
        {
            var httpResponse = await _httpClient.PostAsJsonAsync("api/Authentication/validate-otp", validateOtpRequest);
