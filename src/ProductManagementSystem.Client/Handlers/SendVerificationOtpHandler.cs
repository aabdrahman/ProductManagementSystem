...
public class SendVerificationOtpHandler
{
    ...
    public SendVerificationOtpHandler(IHttpClientFactory httpClientFactory, IConfiguration configuration)
    {
    ...
    public async Task<(bool isSuccessful, string responseMessage)> Handle(SendOtpRequestDto sendOtpRequest)
    {
        try
        {
            var httpResponse = await _httpClient.PostAsJsonAsync("api/Authentication/send-otp", sendOtpRequest);
